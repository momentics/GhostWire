...
| | |
|:---:|---|
| <img src="resources/icons/ghostwire.png" alt="GhostWire Logo" width="512"> | **GhostWire Desktop** помогает **Telegram Desktop** работать через локальный защищённый прокси. Приложение запускает SOCKS5 на вашем компьютере, подключает к нему Telegram и передаёт трафик через **HTTPS Tunneling** с защитой от DPI на уровне **L4-L7**. Системный VPN, внешние прокси-сервисы и сторонние аккаунты не нужны. |
...
4. Дальше трафик уходит через `HTTPS` на порту 443, то есть выглядит как обычное защищённое Web соединение.
5. Если маршрут недоступен, библиотека может переключиться на другой маршрут для того же направления.
...
- **HTTPS Tunneling** - исходящие соединения используют порт `443`, привычный для HTTPS-трафика.
- **Ротация маршрутов** - при проблемах с одним маршрутом GhostWire может использовать другой доступный endpoint.
...
- **DC-aware routing** - GhostWire сопоставляет Telegram IP-диапазоны с нужными дата-центрами
- **Endpoint rotation** - для дата-центров указано несколько основных и резервных endpoint'ов
- **Media routing** - для части направлений выделены отдельные endpoint'ы и доменные имена для медиа-трафика
...
- Доступна проверка обновлений, но обновления не устанавливаются автоматически. Необходимо подтверждение пользователя.
...
- **Первый запуск** - GhostWire находится в остановленном состоянии (**Стоп**).
